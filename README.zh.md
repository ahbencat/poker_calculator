...
玩家选择手牌与公牌，实时计算每位玩家的 胜 / 平 / 负 概率。胜率来自**精确枚举**
（含翻牌前，无抽样误差），重局面自动切换蒙特卡洛；纯**原生 JavaScript** 在浏览器
本地完成全部计算——零依赖、无构建、可离线（双击 `index.html` 即用）。
界面支持中/英文切换（顶栏按钮）。
...
js/app.js               UI 状态机、分片调度（MessageChannel）、重算抑制、中英双语文案
...
**关键词**：德州扑克 · 德克萨斯扑克 · 胜率计算器 · 概率计算 · 胜率 · 牌力 ·

**关键词**：德州扑克 · 德克萨斯扑克 · 胜率计算器 · 概率计算 · 胜率 · 牌力 ·
Texas Hold'em · NLHE · poker odds calculator · equity · win rate ·
蒙特卡洛模拟 · 精确枚举 · 原生 JavaScript · 零依赖 · 离线可用
