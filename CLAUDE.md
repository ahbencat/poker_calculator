...
```bash
node test/run_tests.js          # 全部 JS 测试（28 项，无测试框架，自建 test/assert）
node test/bench.js              # 性能基准（改热路径前后对比；在枚举上下文测，勿用随机手牌微基准）
...
- **UI 计算门槛、参与规则与重算抑制**：app.js 在**至少 2 位玩家手牌齐全（2 张）**时启动计算（`completeIndexes()` 门控，不论桌人数）；只对手牌齐全的玩家计算，未齐玩家完全不参与（其已选牌不算死牌、留在牌堆里）。重算由**输入签名**（齐牌玩家手牌 + 公牌，`calcSignature`）判定：增删未参与玩家、给新玩家选第 1 张牌等不改签名的操作不触发重算、不打断进行中的任务。因此页面交互只会走精确枚举（MC 仅作重枚举预览），MC 主路径与全随机分支仅作为引擎 API 保留（测试覆盖；引擎的 `opts.deadCards` 死牌能力同样保留）。

**7 张牌评估器**（js/evaluator.js）：掩码状态机（s1–s4 = 恰好出现 k 次的 rank 集合，无分支位运算推进）+ 花色车道计数（4 个花色计数装进一个 int 的 4 字节，无同花时零掩码构建）+ 3 张 8192 项表（STR/POP/TOP5），分数 `(cat<<20)|踢脚位` 可直接比较。7 张牌内同花不可能与四条/葫芦共存，故同花最先判定。**7 张参数的手工展开与无分支写法是刻意为之的热路径优化**（枚举上下文实测 ~1950 万次/秒；if 链版 ~1600 万，随机手牌微基准下差距更大）——改动前先基准测试，且必须在枚举上下文（相邻局面共享 5 张牌、分支更可预测）而非随机手牌微基准下对比。测试向量（V1–V6）是 JS 与 Python 双实现对拍的精确值，由 `test/oracle.py` 可独立复核；校验和 `37575761920` 依赖牌编码顺序，改编码必须同步 oracle.py。
