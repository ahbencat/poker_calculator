...
单次调用零内存分配，枚举上下文实测吞吐约 **1950 万次/秒**（Node 24；
随机手牌微基准约 2000 万——枚举中相邻局面共享 5 张牌、分支更可预测，
两个语境的数字不可互换）。
...
### 4.3 逐张处理（7 次重复，手工展开，全程无分支）

每张牌 `c` 做两件事（`x = 1 << (c >> 2)`）：

**rank 出现次数状态机**（a/b/c = x 当前所在层，无分支位运算推进）：

```
a = x & s1; b = x & s2; c = x & s3
s1 = (s1 | x) & ~(a | b | c)   // x 未出现过才留在 s1；从 s1 升层则移除
s2 = (s2 | a) ^ b              // 从 s1 升入 s2 / 从 s2 升层移除
s3 = (s3 | b) ^ c              // 从 s2 升入 s3 / 从 s3 升层移除
s4 |= c                        // 第 4 张到达 s4，永不离开
```

随机牌局下 if 链的分支误预测不可忽视——无分支版比 if 链快约 13%（随机手牌
微基准），故刻意用位运算选择替代条件跳转。

**花色车道计数**（同样无分支）：

```
sc += INC[c & 3]    // INC = [1, 0x100, 0x10000, 0x1000000]，
                    // 4 个花色的计数装进一个 int 的 4 个字节
```

处理完 7 张后一次性判同花：某花色计数 ≥5 ⇔ 该字节加 3 后 bit3 置位
（计数 ≤7，+3=10 不跨字节进位）：

```
f = (sc + 0x03030303) & 0x08080808
f ≠ 0 → 存在同花（7 张内至多一个花色 ≥5）：
        定位花色 suit，重扫 7 张牌重建该花色掩码 fm（慢路径，仅约 3% 的手）
f = 0  → 无同花，零掩码构建成本
```

全体 rank 掩码 `rm = s1 | s2 | s3 | s4` **延迟派生**（同花早退路径不需要它）。
...
```mermaid
flowchart TD
    S["状态: s1..s4, sc(花色车道计数)"] --> FL{"(sc + 0x03030303) & 0x08080808 ≠ 0 ?"}
    FL -->|是| SF{"STR[重建的花色掩码] ≠ 0 ?"}
...
### 5.2 实测耗时（本机 Node 24，同进程交替 A/B）

| 场景 | 局面数 | 评估次数 | 耗时 |
|---|---|---|---|
| 2 人翻牌前 | 1,712,304 | 342 万 | ~180ms |
| 5 人翻牌前（最重） | 850,668 | 425 万 | ~220ms |
| 10 人翻牌前 | 201,376 | 201 万 | ~105ms |
...
> 注：UI 侧另设计算门槛与参与规则（`app.js`）——**至少 2 位玩家手牌齐全**即
> 启动计算（不论桌人数），且**只对手牌齐全的玩家计算**；未齐玩家完全不参与，
> 其已选牌不算死牌、留在牌堆中。重算由输入签名（齐牌玩家手牌 + 公牌）判定，
> 增删未参与玩家、给新玩家选第 1 张牌等不改签名的操作不触发重算。
> 故页面交互只会走精确枚举（蒙特卡洛仅作重枚举预览）；MC 主路径、全随机分支
> 与引擎的死牌能力（`opts.deadCards`）作为公共 API 保留，由测试覆盖。
...
| 量 | 定义 | 精确枚举 | 蒙特卡洛 |
|---|---|---|---|
| **equity（期望分池份额）** | 单独获胜概率 + 平局均分期望 | 精确 | 估计值（±1.96·SE） |
| **win（胜）** | 独赢概率 | 精确 | 估计值 |
...
注意 equity 是期望值口径：平局时赢家各得 1/w 而非 1，故 equity = win + Σ(1/w·1(平局))
≤ win + tie。**页面展示不使用期望值**，而是原始概率三口径：

- 进度条三段：胜（绿）/ 平（金）/ 负（底色灰）
- 条下数字：大字为胜，小字「胜 x% · 平 y% · 负 z%」，三者相加恒为 100%
  （负 = 100% − 胜 − 平）

引擎结果中的 equity 字段仍按上表定义保留（测试与对拍的守恒口径），页面不展示。
...
|---|---|---|
| 精确枚举 | O(C(deckSize,k) × P) 次评估 | 2 人翻牌前 ~180ms / ~0.6s |
| 蒙特卡洛 | min(时间预算 650ms, SE 达标, 40 万次) | 2 人半填 ~400 万次/秒 |
| 单次评估 | O(1)，零分配，无分支 | ~51ns（1950 万次/秒） |
